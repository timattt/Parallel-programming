	...
	while (1) {
		...
		if (!get_bit(next)) {
			if (cur_hash > hash_code
					|| (cur_hash == hash_code && cur_key == key))
				goto done;

			prev = &get_node(cur)->next;
			set_hazardous_pointer_with_mask(cur, 2);
		} else {
			if (atomic_compare_and_swap(prev, mk_node(get_node(cur), 0),
					mk_node(get_node(next), 0)))
...
struct node * list_insert_hp(struct hashTable *ht, unsigned bucket, struct node *node) {
	...
	while (1) {
		...
		if (atomic_compare_and_swap(prev, mk_node(get_node(res), 0),
				mk_node(node, 0)))
			return node;
	}
}
